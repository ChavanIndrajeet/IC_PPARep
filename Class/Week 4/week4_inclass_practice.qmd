...
census_api_key(Sys.getenv("9c944699f65b68893c6c529610f655faeec5d440"))
...
my_county <- pa_counties %>%
  filter(COUNTY_NAM == "DAUPHIN")  # Change "CENTRE" to your county. Thus, using DAUPHIN County for my exercise
...
p1 <- ggplot() +
  ...
  geom_sf(data = my_neighbors, fill = "lightcoral", color = "white", alpha = 0.7) +
  geom_sf(data = my_county, fill = "#624948", color = "white") +
  labs(
...
p2 <- ggplot() +
  geom_sf(data = pa_counties, fill = "lightgray", color = "white") +
  geom_sf(data = intersecting_counties, fill = "lightblue", color = "white", alpha = 0.7) +
  geom_sf(data = my_county, color = "Yellow") +
  labs(
    title = paste("Intersecting Counties of", intersecting_counties$COUNTY_NAM[1], "County"),
    subtitle = paste(nrow(intersecting_counties), "intersecting counties")
  ) +
  theme_void()
...
**The difference of 1 clearly shows the way commands operate. `st_intersects` command includes Dauphin County as technically the county intersects with itself. Whereas the `st_touches` command calculates all the counties that touch the boundary in consideration, in this case Dauphin County.**
...
my_county_hospitals <- hospitals_with_county %>%
  filter(COUNTY_NAM == "DAUPHIN")  # Change to match your county
...
```{r}
ggplot() +
  geom_sf(data = hospital_service_areas, fill = "lightblue", color = "white",alpha = 0.4) +
  geom_sf(data = my_county_hospitals, color = "red", size = 2) +
  geom_sf(data = my_county, fill = NA, color = "black") +
  labs(
...
**The Dauphin County is well served as the hosptials in the county cover 66.1% of the total area through their hospital services. The northern part of the county is underserved. We will have to analyse the neighboring counties to ensure if these areas are actually underserved or covered by hospitals located in neighboring counties** 
...
The error in areas increases from South to North. The highest errors are seen in the northen part of the state while using geographic coordinates (WGS84). This is because the WGS84 is more accurate for projecting areas closer to the south of Pennsylvania State as represented by the visualisation
