...
- Rows: 50,000
- Columns: 7 
- Variable types: Numeric , Character (categorical)
- Problematic names: Engine Size, Fuel Type, Year of Manufacturing. They contain spaces, which makes it difficult to reference them directly in R without using backticks. A better approach is to replace spaces with underscores for improved readability and ease of use.
...
**Your answer:** Tibbles print only the first few rows and columns and include column types at the top, while data frames print the entire dataset and do not show column types by default.
...
**Your answer:**  Backticks are required for column names that contain spaces or special characters. The new name `year` does not contain spaces, so backticks are not needed.
...
```
**Question:** Average price by Manufacturer?
...
**Question:** Average mileage by fuel type?
...
**Question:** Frequency table for price categories?
